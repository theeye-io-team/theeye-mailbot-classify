...
module.exports = {
  ...
  buildCacheName (date, startOfDay, tz) {

    const currentDate = date ? DateTime.fromISO(date).setZone(tz) : DateTime.now().setZone(tz)
    const startDate = this.timeExpressionToDateLuxon(startOfDay, tz, currentDate.toJSDate())
