...
class TheEyeFile {
  ...
  static FetchAll = async () => {
    const options = {
        ...
        path: `/${CUSTOMER}/file?access_token=${this.access_token}`,
        method: 'GET'
    }
  ...
  static GetById = async (id) => {
    const options = {
        host: BASE_URL.split('https://')[1] || BASE_URL.split('http://')[1],
        port: (/https:\/\//gi).test(BASE_URL) ? 443 : 80,
        path: `/${CUSTOMER}/file/${id}?access_token=${this.access_token}`,
        method: 'GET'
    }
    ...
    if(!filename && !id) {
      ...
      const downloadFile = async (id) => {
        const options = {
            host: BASE_URL.split('https://')[1] || BASE_URL.split('http://')[1],
            port: (/https:\/\//gi).test(BASE_URL) ? 443 : 80,
            path: `/${CUSTOMER}/file/${id}/download?access_token=${this.access_token}`,
            method: 'GET'
        }
